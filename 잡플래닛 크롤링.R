...
resp <- POST(url = URI,
             body = list('user[email]' = '본인의 계정을 입력',
                         'user[password]' = '본인의 비밀번호를 '))

# 응답 상태코드를 확인, 200이면 정상
status_code(x = resp)
...
url_company = '잡플라넷 기업 면접 페이지'
# 예) https://www.jobplanet.co.kr/companies/000000/interviews
resp = GET(url=url_company, config = list(cookies = myCookies))
status_code(x=resp)
...
#csv로 저장 및 불러오기, 확인
write.csv(df_interview,'경로 및 파일명')   # 예) 'C:/Work_R/Others/interview.csv'
data = read.csv('경로 및 파일명')
View(data)
